...
<div class="max-w-2xl mx-auto">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Edit Tipe Deposito</h2>
        <p class="text-gray-500 text-sm mt-1">Perbarui informasi tipe deposito.</p>
    </div>

    <form action="{{ route('deposito-types.update', $depositoType->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        
        <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nama Tipe</label>
            <input type="text" name="name" id="name" value="{{ $depositoType->name }}" class="focus:ring-primary-500 focus:border-primary-500 block w-full sm:text-sm border-gray-300 rounded-lg py-3" placeholder="Contoh: Deposito Gold" required>
        </div>

        <div>
            <label for="yearly_return" class="block text-sm font-medium text-gray-700 mb-1">Bunga Tahunan / Yearly Return (%)</label>
            <div class="relative rounded-md shadow-sm">
                <input type="number" step="0.01" name="yearly_return" id="yearly_return" value="{{ $depositoType->yearly_return }}" class="focus:ring-primary-500 focus:border-primary-500 block w-full pr-12 sm:text-sm border-gray-300 rounded-lg py-3" placeholder="0.00" required>
